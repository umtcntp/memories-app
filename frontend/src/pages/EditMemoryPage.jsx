...
function EditMemoryPage() {
    ...
    async function fetchMemory() {
        try {
            const response = await axiosClient.get(`/memories/${id}`);
            ...
            setFormData({
                ...
                coverImageFileId: memory.coverImageFileId || "",
            ...
            console.error(error);
            setErrorMessage(
                error.response?.data?.message || "Anı bilgileri alınırken hata oluştu."
            );
        } finally {
            setLoading(false);
        }
    ...
    async function handleSubmit(event) {
        ...
        try {
            ...
            await axiosClient.put(`/memories/${id}`, payload);

            navigate("/admin/memories");
        } catch (error) {
            console.error(error);
            setErrorMessage(
                error.response?.data?.message || "Anı güncellenirken hata oluştu."
            );
        } finally {
            setSubmitting(false);
        }
    ...
    if (loading) {
        return <p className="text-green-700">Anı bilgileri yükleniyor...</p>;
    }
        ...
        <section>
            <header className="mb-6 rounded-[2rem] border border-green-100 bg-white/80 p-8 shadow-sm">
                ...
                <p className="mt-4 text-green-700">
                    Seçili anının başlık, açıklama, tip, Drive dosya ID’si ve diğer
            ...
            >
                ...
                <div className="mt-8 flex justify-end gap-3">
                    ...
                    >
                        {submitting ? "Güncelleniyor..." : "Anıyı Güncelle"}
                    </button>
                </div>
            </form>
        </section>
